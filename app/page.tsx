...
export default function Home() {
  ...
  const handleScoreChange = (game: keyof typeof showLoveLetters, score: number) => {
    if (
      (game === "snake" && score >= 10) ||
      (game === "quiz" && score >= 4) ||
      (game === "memory" && score >= 8) ||
  ...
  <p className="mt-2 text-sm text-rose-400 italic">
    "In all the world, there is no heart for me like yours. In all the world, there is no love for you like mine."
        ...
        <section className="bg-white rounded-lg shadow-lg p-6 mb-12">
          <h2 className="text-2xl font-semibold text-rose-600 mb-4">Love Snake Game</h2>
          <SnakeGame onScoreChange={(score) => handleScoreChange("snake", score)} />
        </section>

        <section className="bg-white rounded-lg shadow-lg p-6 mb-12">
          <h2 className="text-2xl font-semibold text-rose-600 mb-4">Love Quiz</h2>
          <LoveQuiz onScoreChange={(score) => handleScoreChange("quiz", score)} />
        </section>

        <section className="bg-white rounded-lg shadow-lg p-6 mb-12">
          <h2 className="text-2xl font-semibold text-rose-600 mb-4">Memory Match</h2>
          <MemoryMatch onScoreChange={(score) => handleScoreChange("memory", score)} />
        </section>

        <section className="bg-white rounded-lg shadow-lg p-6 mb-12">
          <h2 className="text-2xl font-semibold text-rose-600 mb-4">Love Word Scramble</h2>
          <WordScramble onScoreChange={(score) => handleScoreChange("scramble", score)} />
        </section>
